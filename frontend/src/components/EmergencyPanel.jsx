...
export default function EmergencyPanel({ events = [] }) {
  // Use live hook, initializing with any existing events if provided
  const { requests, activityFeed, assignRequest } = useLiveRequests(events);
    ...
    <div className="h-full flex flex-col pt-1">
      ...
      <div className="emergency-list flex-1 overflow-y-auto pr-2 space-y-3 mb-4">
        {requests.length === 0 ? (
          ...
          requests.map(evt => <EventCard key={evt.id} event={evt} onAssign={assignRequest} />)
        )}
      </div>
